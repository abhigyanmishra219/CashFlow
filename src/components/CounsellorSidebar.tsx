...
export default function CounsellorSidebar() {
  ...
  const groups: SidebarGroup[] = [
    {
      ...
      items: [
        ...
        {
          name: "Enquiries",
          href: "/counsellor-dashboard/enquiries",
          icon: (
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="h-5 w-5">
  ...
  return (
    <aside
      ...
      className={`bg-white border-r border-slate-200/80 h-screen flex flex-col py-6 font-sans transition-all duration-300 shrink-0 ${
              ...
              {group.items.map((item) => {
                ...
                return (
                  <li key={item.name}>
                    <Link
                      href={item.href}
                      className={`group flex items-center ${isCollapsed ? "justify-center" : "gap-3 px-3"} py-2.5 text-xs font-bold rounded-xl transition-all duration-200 relative ${
                        isActive
