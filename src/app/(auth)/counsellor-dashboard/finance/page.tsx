...
export default function CounsellorFeeCollectionPage() {
  ...
  const fetchRecentAdmissions = async () => {
    try {
      ...
      if (res.ok && data.success) {
        // Filter to only this counsellor's admissions
        const scoped = data.data.filter((student: AdmissionType) => {
          if (!user) return false;
          return (student.counsellor || "").toLowerCase().trim() === (user.name || "").toLowerCase().trim();
        });
        setRecentAdmissions(scoped);
      }
    } catch (err) {
  ...
  const handleSearchChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) {
        // Filter search results to only this counsellor's admissions
        const scoped = data.data.filter((student: AdmissionType) => {
          if (!user) return false;
          return (student.counsellor || "").toLowerCase().trim() === (user.name || "").toLowerCase().trim();
        });
  ...
  const generateInstallments = () => {
    ...
    const baseAmount = Math.floor(selectedStudent.finalFee / totalInst);
    
    let runningPaid = totalPaid;
    const installments = [];
    const baseDate = new Date(selectedStudent.admissionDate || "2026-05-16");

    for (let i = 1; i <= totalInst; i++) {
      const instAmount = i === totalInst 
        ? selectedStudent.finalFee - baseAmount * (totalInst - 1) 
        : baseAmount;
                ...
                <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-xs space-y-5">
                  ...
                  <form onSubmit={handlePaymentSubmit} className="space-y-5 text-xs font-bold text-slate-600">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-slate-400 mb-1.5">Company *</label>
                        <select
                          ...
                          className="w-full text-xs font-semibold text-slate-700 bg-white border border-slate-200 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-indigo-500/50"
                        >
                          <option value="Design Gateway Pvt Ltd">Design Gateway Pvt Ltd</option>
                          <option value="SyncForge Solutions">SyncForge Solutions</option>
                          <option value="Apex Elite Coaching">Apex Elite Coaching</option>
                        </select>
                      </div>
                    ...
                    {installments.map((inst, index) => (
                      <div key={index} className="relative flex justify-between items-start text-xs font-bold">
                        ...
                        <div className="space-y-0.5 pr-2">
                          <span className="block text-slate-800 leading-none">{inst.num}</span>
                        ...
                        <div className="text-right space-y-1">
                          ...
                          <span className={`inline-block text-[8px] font-extrabold border rounded-md px-1.5 py-0.2 uppercase leading-none ${inst.statusClass}`}>
                            {inst.status}
                          </span>
                        </div>
                      </div>
